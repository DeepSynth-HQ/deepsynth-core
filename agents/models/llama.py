...
class Metrics:
    ...
    def log(self):
        ...
        if self.completion_tokens_details is not None:
            logger.debug(
                f"* Completion tokens details:   {self.completion_tokens_details}"
            )
...
class LlamaChat(Model):
    ...
    def format_message(
        self, message: Message, map_system_to_developer: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        if message.role == "assistant":
            message.content = (
                "Assistant" if message.content is None else message.content
            )
        ...
        if self.response_format is not None and self.structured_outputs:
            try:
                if isinstance(self.response_format, type) and issubclass(
                    ...
                    formated_messages = [self.format_message(m) for m in messages]  # type: ignore
                    logger.info("START FORMATED MESSAGES")
                    logger.info(formated_messages)
                    logger.info("END FORMATED MESSAGES")
